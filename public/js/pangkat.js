document.addEventListener("DOMContentLoaded", function(){
    ...
    $('#datatable tfoot th').each(function (index) {
        ...
        if(['golongan'].includes(name)){
            $(this).html(`
                <div class="form-group mb-0 pr-4">
                    <input type="text" class="form-control text-center" name="${name}" id="${name}-form">
                </div>
            `);
        }else if(['tmt'].includes(name)){
            $(this).html(`
                <div class="form-group mb-0 pr-4">
                    <input type="date" class="form-control text-center" name="${name}" id="${name}-form">
                </div>
            `);
        }else{
            $(this).text('');
    ...
    $('#datatable').on('draw.dt', function (datatable) {
        ...
        for(let button of cancelButtons){
            button.addEventListener('click', function(e){
                e.preventDefault();
                var pangkatId = e.target.id.split('-')[2];

                confirmAlert(
                    "Konfirmasi Hapus Pangkat",
                    "Apakah Anda Yakin ingin Menghapus Pangkat?",
                    async function(){
                        const response = await fetch(
                            `${window.location.href}/${pangkatId}/delete`,
